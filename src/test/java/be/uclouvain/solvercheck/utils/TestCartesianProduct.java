...
public class TestCartesianProduct implements WithSolverCheck {
    ...
    public void itMustHoldATupleForEachCombinationOfValues(){
       ...
       .assertThat(sets ->
        forAll(listOf(integer().between(0, Integer.MAX_VALUE)).ofSize(sets.size()))
    ...
    public void containsReturnsFalseForAnyCombinationWithNonItems() {
        assertThat(forAll(listsOfDomains(), integer())
    ...
    public void getFailsWhenIIsLessThanZero() {
        assertThat(forAll(listsOfDomains(), integer().between(MIN_VALUE, 0))
            .assuming((sets, i) -> i < 0)
            .itIsTrueThat((sets, i) ->
                failsThrowing(
    ...
    public void getFailsWhenIIsLessBiggerThanSize() {
        assertThat(forAll(listsOfDomains(), integer().between(0, Integer.MAX_VALUE))
