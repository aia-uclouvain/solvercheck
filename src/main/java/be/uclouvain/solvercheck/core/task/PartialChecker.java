 ...
 *
 * .. Example::
 *    An example from where such a partial checker might be useful, is the case from
 *    anti-monotonic constraints st `allDifferent`. Indeed, a smart checker for the
 *    `allDiff` constraint should be able to detect that no extension from the given
...
public interface PartialChecker extends Checker {
    ...
    /**
     * This enumeration defines the possible outcomes from a check applied to some partial
     * assignment.
     *    - ACCEPT means that the checker established that all extensions from the current
     *             partial assignment will be accepted by the constraint.
     *    - REJECT means that the checker established that all extensions from the current
