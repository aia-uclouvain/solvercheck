 ...
 *    to always be of type integer.
 *
 * .. Note::
 *    Partial assignments form a partial order. The lattice of partial ordering over partial
 *    assignements is defined as follows:
 *       - A is EQUIVALENT   to B: iff A and B have the same size and the domains of all variables
 *                                 of A and B are EQUIVALENT.
 *       - A is STRONGER   than B: iff A and B have the same size and the domains of all variables
 *                                 of A are either STRONGER or EQUIVALENT to those of B.
 *       - A is WEAKER     than B: iff B is STRONGER than A.
 *       - A is INCOMPARABLE to B: if
 *             + A and B do not have the same size, or
 *             + There exists some variable i such that the domains of i in A and B are INCOMPARABLE, or
 *             + There exists some variables i and j such that the domain of i in A is STRONGER
 *                than that of i in B **AND** the domain of j in A is WEAKER than that of j in A.
 */
public interface PartialAssignment extends List<Domain>, PartiallyOrderable<PartialAssignment> {
    ...
    /**
     * A partial assignment is a leaf iff it occurs at the very bottom of a search tree.
