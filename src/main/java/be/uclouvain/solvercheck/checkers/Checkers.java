...
/**
 * This class is meant to be used as a factory to instantiate checkers for the most common
 * constraints. Namely, it lets you create checkers for the following constraints:
 *   - true, false               :: Not real constraints: they always return the same value
 *   - allDiff(x_1, ..., x_n),   ::
 *   - sum(op, var)              :: This covers sumEq, sumNe, sumLe, sumGe, sumLt, sumGt
 *   - element(xs, index, value) ::
 *   - table,                    ::
 *   - gcc, gccVar               :: The nuance between the two being that gcc requires
 *                               :: values and cardinalities to be fixed and provided at instantiation
 *                               :: time, whereas gccVar only requires values. The cardinalities being
 *                               :: interpreted as the last |values| variables of the given partial
 *                               :: assignment.
 *
 * FIXME: Question, veut-on garder les checkers comme une série de fonctions statiques ?
 *        ou veut-on en faire des classes ? (Elles seraient simples, cohésives, pas couplées + ca serait
 *        sans doute plus consistant avec ce qui est fait pour les consistances)
 */
public final class Checkers {
    ...
    /**
     * A checker that tells whether all variables of the given assignment satisfy the
     * allDifferent constraint. In other words, this checker evaluates an assignment to
     * true iff all the variables of the assignment take a different value.
     *
     * @return a checker validating if a solution satisfies the alldiff constraint
     */
    ...
    /**
     * A checker that tells whether the sum of the values of all variables of the given
     * assignment satisfy some given linear condition.
     *
     * .. Example::
     ...
     *
     * .. More formally::
     *    This checker evaluates true iff the assignment satisfies the given expression:
     *    $$
     *    \sum_{i = 0}^{|assignment|} assignment[i] \otimes constant
     *    $$
     *    where $\otimes$ is one of the following operators: $<, \le, =, \ne, \ge, >$
     *
     * @param op the operator determining the condition that applies to the total of the sum
     * @param constant the constant that imposes some value constraint on the sum
     * @return checker that tells whether the sum of the values of all variables of the given
     *      assignment satisfy some given linear condition.
     */
     ...
     *
     * @return a checker verifying that the element constraint is satisfied by the given assignment.
     */
    ...
    /**
     * This method parses the assignment so as to be able to interpret the assignment as a candidate
     * solution to an element constraint.
     *
    ...
    /**
     * This is the actual implementation of the checker for the element constraint. It is satisfied
     * iff:
     *   - the given index is *valid* (in range [0..variables.size()[ ) **AND**
     *   - variables[index] == value.
     *
     * @param variables the variables that have been assigned in the candidate solution
     * @param index the index of the variable whose value is being constrained
     * @param value the value which variable[index] should have for this checker to evaluate true
     *
    ...
    /**
     * A checker that returns true iff the given assignment belongs to the given table of possible assignments
     * (This is the typical case of an extensional constraint).
     *
     * @param table the table comprising all possible solutions.
     * @return true iff the given assignment belongs to the given table of assignments
     */
     ...
     *
     * @param cardinalities the list of cardinality for each value. (The number of time each value[i] must occur
     *                      in the evaluated assignment.
     * @param values the list of values that must occur with certain cardinalities in the assignment.
     * @return a checker validating that some assignment verifies the gcc constraint.
     */
    ...
    /**
     * This is the actual implementation of the gcc checker. It returns true iff for all i, the value[i]
     * occurs cardinalities[i] times in the list of variables.
     *
     * @param variables the variables that have been assigned some value
     * @param cardinalities the cardinalities that must be forced onto the variables for each value
     * @param values the values that must occur in the list of variables.
     *
     * @return true iff for all i, the value[i] occurs cardinalities[i] times in the list of variables.
     */
