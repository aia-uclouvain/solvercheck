...
/**
 * This class lets an user build a Range consistent Filter from some
 * given Checker. This means that domains will be filtered to only contain
 * values belonging to some **bound support** of the constraint.
 */
public class RangeConsistency extends AbstractUniformConsistency {
     ...
     * It returns a domain filter that ensures that all values of the filtered domain
     * have a **bound support** in the domain of the other variables.
     *
     * @param checker the checker testing the satisfaction of the constraint
     * @return a DomainFilter that ensures the Range consitency of the given checker for some
     *         variable domain.
     */
