...
/**
 * An hybrid consistency is one that does not uniformly applies the same DomainFilter to all of the variables.
 * Instead, the HybridConsistency class lets you specify what domain filter to apply for each variable.
 *
 * Just like any other consistency algorithm, this class lets you build a Filter from some given checker and
 * computes the filtered partial assignment as the least fixpoint of the domains filtered by the DomainFilters.
 */
public class HybridConsistency implements Filter {
    ...
    /**
     * Creates a new hybrid consistency from some given checker and a list of function producing domain filters.
     *
     * .. Note::
     *    The given list of function producing the domain filters **must** have the same length as the arity
     *    of the constraint being checked. Any failure to comply with this rule will result in the hybrid
     *    consistency filter throwing an exception.
     *
     * @param checker the checker embodying the constraint being verified
     * @param domainFilter a vararg list of function that can be used to produce domain filters from the given
     *                     checker.
     */
    @SafeVarargs
